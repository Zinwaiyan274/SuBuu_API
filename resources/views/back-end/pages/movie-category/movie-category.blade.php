        ...
        <div class="tab-content order-summary-tab">
            <div class="tab-pane fade show active" id="ctg-list">
                <div class="table-header">
                    <h4>{{ __('Movie categories List') }}</h4>
                </div>
                <div class="row mt-3">
                    <div class="col-sm-4">
                        <form action="">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-check form-switch">
                                            <input class="form-check-input status-item" type="checkbox"  name="status_{{$category->id}}" id="status_{{$category->id}}" value="{{$category->status}}"  {{$category->status==1?'checked':''}} data-id ="{{$category->id}}" data-status="Movie Category Status" >
                                        </div>
                                    </td>
                                    <td>
                                        <div class="dropdown table-action">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li>
                                                    <a href="{{ route('edit-movie-category', ['id' => $category->id]) }}">
                                                        <i class="fal fa-pencil-alt"></i>
                                                        {{ __('Edit') }}
                                                    </a>
                                                </li>
                                                <li>
                                                    <a href="{{ route('delete-movie-category',$category->id) }}" class="confirm-action" data-method="DELETE">
            ...
            <div class="tab-pane fade" id="ctg-add">
                <div class="table-header">
                    <h4>{{ __('Create Movie cateogry') }}</h4>
                </div>
                <div class="row">
                    <div class="col-sm-8">
                        <div class="modal-body order-form-section">
                            <form action="{{ route('new-movie-category') }}" method="post" class="ajaxform_instant_reload" enctype="multipart/form-data">
                                @csrf
